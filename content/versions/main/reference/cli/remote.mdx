...
`--web-port <port>` pins the local browser origin; otherwise RimZ derives a stable port from the session name in `8300..8399`. The room itself is [`rimz web`](/docs/main/reference/cli/web).

Link health, web tunneling, reconnect mechanics, and bandwidth attribution are in [remote internals](https://github.com/rimio-ai/rimz/blob/main/docs/internals/remote.md?ref=docs).
