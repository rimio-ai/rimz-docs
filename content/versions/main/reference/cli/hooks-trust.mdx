...
Installed hooks call back into RimZ for lifecycle and blocking-ask events. Hook stdout is the agent's decision channel, so installed hooks keep diagnostics off stdout and return only the agent-native neutral no-op for blocking asks; the prompt stays in the agent UI ([the adapter boundary](https://github.com/rimio-ai/rimz/blob/main/docs/internals/agents/model.md?ref=docs#the-adapter-boundary)). Some agents add their own hook trust gate; when one reports installed-but-untrusted hooks, `rimz doctor` prints the exact fix.
...
Project trust covers project-supplied command surfaces: hook commands, agent launch commands, profile and team definitions, env overrides, and other executable fields. Until you grant trust, none of these run. The hash, stored surface, and record format are in [project trust](https://github.com/rimio-ai/rimz/blob/main/docs/internals/harness/trust.md?ref=docs); the operator-facing safety model is in [security and trust](/docs/main/help/security).
