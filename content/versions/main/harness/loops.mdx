...
`--check` gates firing; `--verify` gates completion. Add `--verify "cargo test"` to a scheduled `--agent` task when the command is the definition of done: a red result returns its evidence to the same live session, up to `--max-attempts` total turns, before the fire records `verify failed`. It behaves exactly as it does on a hand-run `-p`, described in full under [verify and retry](/docs/main/harness/scripting#verify-and-retry).
...
`--surplus` is that condition. Before each fire, RimZ reads the provider's longest pacing window and computes forward headroom, how far ahead of the sustainable pace the window is running: `1.0x` is exactly on pace, and `1.5x` means half again as much budget remains as the clock requires. Which window counts as the pacing window is per provider, and the headroom model, a worked example, and the fail-closed rules live in [budgets → spend the provider surplus](/docs/main/agents/budget#spend-the-provider-surplus).
...
That certified failure marker is what makes recovery a per-provider capability rather than a global switch: an agent whose adapter exposes no marker RimZ trusts keeps its error stops terminal, and a loop fire records the ordinary failure. [Agent support](/docs/main/reference/agent-support#notes-on-the-alpha-and-experimental-set) states it per agent. The backoff and retry keys are in [configuration.md → Resume](/docs/main/customization/configuration#resume); the decision logic is [providers.md → Auto-continue](https://github.com/rimio-ai/rimz/blob/main/docs/internals/agents/providers.md?ref=docs#auto-continue).
...
- [loops.md](https://github.com/rimio-ai/rimz/blob/main/docs/internals/harness/loops.md?ref=docs) — the clock, state files, and run log underneath.
