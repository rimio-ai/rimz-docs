...
**Cap the dollars.** `--budget 2` records `budget_exceeded` and exits `125`, distinct from a timeout or agent failure. The cap model behind the flag is the [budgets guide](/docs/main/agents/budget).

**Honor the exact Qwen account.** A fresh managed Qwen run resolves the final Alibaba region and API key chosen by the launch environment, compares only that opaque account binding with cached Coding Plan windows, and exits `125` before recording the run or opening a pane when any authoritative window is spent. Missing quota data permits the run; a cache entry for another region or key cannot block it. Interactive launches, resume, fork, and wake keep their existing behaviour.
...
**It works your harness.** `--worktree` isolates the run's changes on their own branch, a [profile](/docs/main/agents/fleet#profiles-shape-an-agent-for-one-job) or `--model`/`--effort`/`--system-prompt-file` shapes the turn, [`rimz loop`](/docs/main/harness/loops) fires the same path on a clock, and the run messages your interactive agents — and is messaged by them — like any teammate.

Permissions are a per-run choice, rendered through the agent's own flags: `-p` defaults to the provider's auto-accept mode for routine actions, `--ask` keeps every native prompt (each one routes to you as a waiting row), and `--yolo` passes the provider's bypass flag. The tradeoffs are [Loops → the permission posture for unattended runs](/docs/main/harness/loops#the-permission-posture-for-unattended-runs) and [security.md](/docs/main/help/security).
...
- `rimz message --steer @<agent> "continue"` is the first-class nudge for wrapper scripts; `rimz message @<agent> --on done "open a PR summary"` hands follow-up work to a running agent at its next turn boundary. The full delivery model is [Messaging](/docs/main/harness/messaging).
...
Supervised runs need installed and trusted hooks, because hooks are the completion signal — a run with no hooks has no way to know its turn ended. `rimz doctor` confirms the hook state, and [Troubleshooting](/docs/main/help/troubleshooting) covers a run that never completes. Installing hooks is a one-time consent step, walked in [set up your machine](/docs/main/getting-started/setup#install-agent-hooks).

## See also

- [Loops and schedules](/docs/main/harness/loops) — put these runs on a clock: schedules, watchdogs, and self-waking agents.
- [Notifications](/docs/main/harness/notifications) — the push routes and handlers that reach you when a run needs an answer.
- [Messaging](/docs/main/harness/messaging) — the `--steer` / `--on done` delivery model wrappers lean on.
- [Agents](/docs/main/agents/fleet) — the profile, handle, and layout vocabulary these examples use.
- [Worktrees](/docs/main/harness/worktrees) — the isolated branches behind `--worktree` runs.
- [Agent control CLI](/docs/main/reference/cli/agents#supervised-runs--p) — every flag on `-p`, `wait`, `show`, `stop`, and `pane`.
- [harness.md → Supervised runs](https://github.com/rimio-ai/rimz/blob/main/docs/internals/harness/harness.md?ref=docs#supervised-runs) — run records, the wakeup socket, streaming, and pane cleanup.
