...
The full model is in [loops.md](https://github.com/rimio-ai/rimz/blob/main/docs/internals/harness/loops.md?ref=docs), and the CLI is in [loop.md](/docs/main/reference/cli/loop).
...
Payload-fidelity and retention controls (`[privacy] payload_mode`) are a planned project surface. The design and intended keys are in [security.md](/docs/main/help/security), and the hook boundary they will govern is in [adapter.md → The hook path](https://github.com/rimio-ai/rimz/blob/main/docs/internals/agents/adapter.md?ref=docs#the-hook-path).
