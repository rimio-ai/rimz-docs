...
Every element carries its state by shape first (the [glyph legend](https://github.com/rimio-ai/rimz/blob/main/docs/interface/sidebar.md?ref=docs#reading-the-glyphs) is the meaning table), so color reinforces meaning rather than carrying it, and any palette stays readable, including no color at all. This page is the knobs; [interface/sidebar.md](https://github.com/rimio-ai/rimz/blob/main/docs/interface/sidebar.md?ref=docs) is what every tone and glyph *means* on screen.
...
Zellij web rooms use the active scheme for the browser terminal when `[web.zellij] style_client` is true; see [Web CLI](/docs/main/reference/cli/web).
...
`color` accepts a palette role, `#rrggbb`, or a raw index. Which blocks appear and in what order is a [Display](#display) and discovery setting (see [configuration.md → Provider dashboard](/docs/main/customization/configuration#provider-dashboard)).
...
The [pets guide](/docs/main/customization/pets) covers the rest: what the pet acts out, the built-in and [petdex.dev](https://petdex.dev/) catalogs, bring-your-own sprite sheets, the pixel and cell-art render tiers, and the offline and privacy story.
