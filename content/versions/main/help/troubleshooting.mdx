...
Doctor lists only the presence plugins currently loaded in the Zellij session, with each plugin's load time, build, Zellij version, and active, rejected, or inactive status. More than one loaded plugin and every rejected row point to `rimz reload`, while a recent increase in topology or other command failures remains a real warning. Full generation history stays in the `plugin-presence.log.jsonl` path doctor prints; when the live Zellij pane listing cannot be read, the section reports that it is unavailable instead of substituting history.
