...
Hook payloads can carry prompts, tool inputs, file paths, command arguments, and errors. They stay in RimZ's local state, and nothing forwards them off the box. Payload-fidelity and retention controls (a `[privacy]` block with `payload_mode` and `retention_days`) are a planned project surface, tracked in [configuration.md](/docs/main/customization/configuration) against the [hook adapter boundary](https://github.com/rimio-ai/rimz/blob/main/docs/internals/agents/adapter.md?ref=docs#the-hook-path).
