...
---
`rimz` opens one room for the project you are in and keeps it attachable. The room is a plain Zellij or tmux session with the sidebar added — your keybinds, layout, and scrollback stay yours — so leaving is a normal multiplexer detach and [`rimz reset`](/docs/v0.2/reference/cli/maintenance#reload-reset-gc-and-uninstall) is the escape hatch if one ever wedges. This page covers the commands that open, reach, and diagnose a local room: `start`, `attach`, `list`, `setup`, and `doctor`. Reaching a room on another host is [`rimz remote`](/docs/v0.2/reference/cli/remote); how RimZ picks the room from your directory is [the root model in ARCHITECTURE.md](https://github.com/rimio-ai/rimz/blob/v0.2/ARCHITECTURE.md?ref=docs).
...
Enable command, flag, and live room completion from the [shell completion setup](/docs/v0.2/getting-started/setup#shell-completion).
...
`--audit` widens the agent section to every observed session, `--json` emits the whole report as one machine-readable document, and `--output PATH` writes it atomically to a file. Static adapter coverage has its own command, [`rimz coverage`](/docs/v0.2/reference/cli/maintenance#adapter-coverage).
