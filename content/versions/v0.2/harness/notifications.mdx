...
---
> When an agent needs you and you are not looking, RimZ taps you: a desktop banner, a terminal bell, and — through notification handlers — any channel or script you own. This page wires the tap to your phone, your chat, or a script that clears routine prompts itself. The trigger model, debounce, and delivery plumbing are [internals → notifications](https://github.com/rimio-ai/rimz/blob/v0.2/docs/internals/sidebar/notifications.md?ref=docs).

## Why notifications

RimZ's job is routing your attention, and the sidebar does it while you watch: a row flips to `? waiting`, rises, and one keystroke lands you in the right pane ([sidebar](/docs/v0.2/agents/sidebar)). The moment that costs you is when you are not watching. You are heads-down in another pane, in a meeting, detached over SSH, asleep — and an agent hits a permission prompt ten minutes into an hour of work. The blocker is a one-line answer; unseen, it stalls the agent for exactly as long as you are away.
...
Two caveats are worth knowing up front. Zellij currently drops desktop notification escapes, so on Zellij the handler path below is the route to a native banner. And when no banner appears anywhere, [troubleshooting](/docs/v0.2/help/troubleshooting#notifications-dont-fire) walks the checklist from terminal support to OS permission.
...
A `when` clause narrows a handler; its present conditions all have to match, and an empty `when` matches everything. `kind` names the notification kinds — the agent statuses plus `coalesced`, `reminder`, `loop_paused` when a scheduled task reaches its strike threshold, and the `link_lost`/`link_restored` pair that [`rimz remote connect`](/docs/v0.2/agents/remote) fires when an SSH link drops or recovers. `worktree` glob-matches the agent's branch, and `handle` glob-matches its handle or role, so the noisy experiment stays quiet while `@planner` on `release/*` reaches your phone.

Handlers live in per-machine config and stay outside project trust, because they are personal routing that often carries host-specific push credentials — a cloned repository never brings its own ([security](/docs/v0.2/help/security)). The payoff of the phone route: a failing migration at 3 a.m. becomes a push notification, you open the room from wherever you are ([remote](/docs/v0.2/agents/remote), [web](/docs/v0.2/agents/web)), answer in the agent's own UI, and the pipeline is green by morning.

For Claude and Codex, [remote control](/docs/v0.2/agents/remote#answer-asks-from-your-phone) closes the same loop through the provider's own app: the room starts the official mobile bridge on your opt-in, the ask pushes to your phone, and you answer the native prompt right there, with no handler to write and no reattach.
...
- `rimz agents <kind> -p` runs a one-shot [supervised turn](/docs/v0.2/harness/scripting) to decide.
...
- [Sidebar](/docs/v0.2/agents/sidebar) — the unread inbox and attention ranking these notifications mirror.
- [Remote → answer asks from your phone](/docs/v0.2/agents/remote#answer-asks-from-your-phone): the provider's own mobile push-and-answer bridge, started with the room.
- [Loops and schedules](/docs/v0.2/harness/loops) — the other half of unattended work: schedules, watchdogs, and the permission posture.
- [Remote](/docs/v0.2/agents/remote) and [web](/docs/v0.2/agents/web) — answering from another machine or a phone once the push lands.
- [Troubleshooting](/docs/v0.2/help/troubleshooting#notifications-dont-fire) — when no desktop banner appears.
- [Configuration](/docs/v0.2/customization/configuration#notifications) — every `[notifications]` key: triggers, debounce, coalescing, reminders, templates.
- [internals → notifications](https://github.com/rimio-ai/rimz/blob/v0.2/docs/internals/sidebar/notifications.md?ref=docs) — the producer/renderer split, the unread model, and the trace log.
