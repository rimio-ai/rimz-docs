...
| `theme.toml` | sidebar appearance: palette, slots, glyphs, animations, provider styling, pets ([theme.md](/docs/v0.2/customization/theme)) |
...
Resume covers two moments; the behavior model is [loops.md → Built-in recovery](/docs/v0.2/harness/loops#built-in-recovery), and these are its keys.
...
The rebirth path is in [sidebar.md](https://github.com/rimio-ai/rimz/blob/v0.2/docs/internals/sidebar/sidebar.md?ref=docs#resume-on-rebirth) and the live path in [provider.md → Auto-continue](https://github.com/rimio-ai/rimz/blob/v0.2/docs/internals/agents/providers.md?ref=docs#auto-continue).
...
These keys are the on-switch; `rimz budget` inspects and adjusts an armed cap at runtime without touching them, and refuses to arm a cap they never set. The full cap model — the per-agent and loop-task scopes, what a park does, and what resumes it — is the [budgets guide](/docs/v0.2/agents/budget).
...
`smart_compact` sets the default threshold for compact-first `message` sends, an occupied-token count (`"200k"` or `"120000"`) or a percentage of the window (`"70%"`). When an agent's context window has reached the threshold, RimZ submits its `/compact` ahead of your text so the prompt lands against a fresh window. Leave it unset to keep compaction opt-in through the per-command `--smart-compact` flag, which overrides this value. The mechanics are in [messaging.md](https://github.com/rimio-ai/rimz/blob/v0.2/docs/internals/harness/messaging.md?ref=docs#smart-compaction).
...
To configure your *own* Zellij or tmux (the theme, true color, copy-mode, and keybindings RimZ leaves to you, and your sessions outside the room) see the [Zellij](/docs/v0.2/customization/multiplexer#zellij) and [tmux](/docs/v0.2/customization/multiplexer#tmux) baselines.
...
Loop tasks live in `~/.config/rimz/loop.toml` under `[tasks.<name>]`; shared project tasks use the same shape in `<repo>/.rimz/config.toml`, are trust-hashed, and stay inert until `rimz trust grant`. The scheduling model (shapes, watchdogs, pings, self-wakes) is [loops.md](/docs/v0.2/harness/loops); this section is the field shape.
...
The full model is in [harness.md → Scheduled turns](https://github.com/rimio-ai/rimz/blob/v0.2/docs/internals/harness/harness.md?ref=docs#scheduled-turns-loop), and the CLI is in [loop.md](/docs/v0.2/reference/cli/loop).

## theme.toml: appearance and pets

The sidebar's palette, glyphs, animations, color depth, color stops, and pets live in `theme.toml`, documented in full in [theme.md](/docs/v0.2/customization/theme). The pointers below cover the settings that touch sidebar behavior and the pet selector.
