...
---
You already cap spend where the provider lets you: a billing alert on the API dashboard, a plan whose included usage refills on a window. Those guard the account. They know nothing about the work: no provider setting says "this experiment is worth $5", "the nightly triage loop gets $20 a day", or "this room stops at $50 a day, whatever I forgot to close before bed". Once the fleet runs unattended — [auto-continue](/docs/v0.3/harness/loops#built-in-recovery) resuming parked turns, loop tasks firing at 02:00 — the gap between what you meant to spend and what the account allows is exactly where the surprise bill lives.

A budget is a dollar cap RimZ enforces at the scale you promise yourself. It can enforce one because it already computes the fleet's live spend from the transcripts your agents write ([Token Insight](/docs/v0.3/agents/insight)); a budget turns that read into a stop. Crossing a cap parks the agent — the same rest state a provider rate limit produces — and a parked agent is one message away from resuming.

One term collision to clear first: the `5h` and `7d` bars on the provider dashboard are also called budgets. Those are your subscription plan's included usage, metered by the provider and read-only to you ([budget is not spend](/docs/v0.3/agents/insight#budget-is-not-spend)), though a [scheduled ping](/docs/v0.3/harness/loops#prime-the-budget-window) can start those windows on your clock. Most of this page is about the dollar caps you set; the [last section](#spend-the-provider-surplus) reads the provider's budget instead, releasing scheduled work only when the week has surplus to spend.
...
1. The room's sidebar process re-checks every scope on its regular tick, against the same transcript-derived spend that [Token Insight](/docs/v0.3/agents/insight#how-the-numbers-are-calculated) reads.
...
- **The day does.** A `/day` park reopens at the next local midnight, and with [auto-continue](/docs/v0.3/harness/loops#built-in-recovery) on, the continue prompt goes to the agents RimZ interrupted — never to agents that were already at rest.
...
The gate is a read, nothing more. The headroom comes from the provider's own usage reporting, the same account-scoped reading that draws the dashboard bars, cached on disk by the sessions you already run. Checking it runs no command and spends no tokens. A closed gate records `surplus skipped` in the task's run history with the reading it saw (`claude 7d window surplus 1.4x below 1.5x`), adds no [strike](/docs/v0.3/harness/loops#built-in-recovery), and the schedule keeps polling until real surplus appears. It also fails closed: a missing, incomplete, expired, or not-yet-started window reading keeps the gate shut, so an API-key account, which has no subscription window to read, never fires a surplus-gated task.

The gate rides loop tasks only, on `--agent` and `--wake` actions, and it is evaluated before any `--check` guard, so a closed gate does not even run the check. The soak-task recipe is [loops → soak up the weekly surplus](/docs/v0.3/harness/loops#soak-up-the-weekly-surplus); the exact flag grammar is the [loop reference](/docs/v0.3/reference/cli/loop).

## See also

- [Token Insight](/docs/v0.3/agents/insight) — reading the spend that budgets act on: the cockpit, the provider dashboard, and `rimz stats`.
- [Loops](/docs/v0.3/harness/loops#built-in-recovery) — auto-continue, task budgets, and hands-off recovery in one loop.
- [Scripting](/docs/v0.3/harness/scripting) — the `-p` exit-code contract, including `125` for a run over budget.
- [Configuration → daily dollar budgets](/docs/v0.3/customization/configuration#daily-dollar-budgets) — the `harness.budget` and `[accounts.budget]` keys.
- [Budget reference](/docs/v0.3/reference/cli/agents#inspect-and-change-a-budget) — the complete `rimz budget` and `rimz agents budget` surface.
- [Providers internals](https://github.com/rimio-ai/rimz/blob/v0.3/docs/internals/agents/providers.md?ref=docs) — spend windows, ledgers, and park mechanics in depth.
