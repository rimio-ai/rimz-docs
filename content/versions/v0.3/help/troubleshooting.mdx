...
`rimz reload` runs from anywhere and leaves stopped sessions stopped. Agent version drift and its exact effects are in [agent support](/docs/v0.3/reference/agent-support).
...
- **Zellij rooms.** Zellij currently drops notification escapes, so `desktop = "auto"` skips them there. For OS-level notifications on Zellij, wire a `[[notifications.handler]]` command (`notify-send`, `ntfy`, or anything else); the shape is in [the configuration guide](/docs/v0.3/customization/configuration#notifications).
...
`--state`, `--config`, and `--all` widen the removal to durable stores and per-machine config; the exact scope of each flag is in the [maintenance reference](/docs/v0.3/reference/cli/maintenance#reload-reset-gc-and-uninstall). Project-local `.rimz/` directories and RimZ-owned worktrees stay in place, because they can hold project config and unlanded work.
...
- [Set up your machine](/docs/v0.3/getting-started/setup) for the first-pass configuration these fixes assume.
- [Security and trust](/docs/v0.3/help/security) for the trust model and the presence grant behind two of the fixes above.
- [Remote](/docs/v0.3/agents/remote) for reconnect behavior and link health.
- [CLI reference: maintenance](/docs/v0.3/reference/cli/maintenance) for every flag on `doctor`, `reset`, `gc`, `reload`, and `uninstall`.
