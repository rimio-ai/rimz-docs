...
| --- | --- | --- |
| **Open and attach a room** | `rimz`, `start`, `attach`, `list`, `setup`, `doctor` | [Getting started](/docs/v0.3/reference/cli/getting-started) |
| **Reach a room anywhere** | `remote`, `web` | [Remote](/docs/v0.3/reference/cli/remote) · [Web](/docs/v0.3/reference/cli/web) |
| **Run and steer agents** | `agents`, `asks`, `answer`, `message`, `transcript`, `pane` | [Agents](/docs/v0.3/reference/cli/agents) · [Asks](/docs/v0.3/reference/cli/asks) · [Message](/docs/v0.3/reference/cli/message) · [Transcript](/docs/v0.3/reference/cli/transcript) · [Pane](/docs/v0.3/reference/cli/pane) |
| **Cost, usage, and budgets** | `stats`, `budget` | [Stats](/docs/v0.3/reference/cli/stats) · [Budgets](/docs/v0.3/reference/cli/agents#inspect-and-change-a-budget) |
| **Lanes, worktrees, and schedules** | `channel`, `worktree`, `loop` | [Channels](/docs/v0.3/reference/cli/channel) · [Worktrees](/docs/v0.3/reference/cli/worktree) · [Loop](/docs/v0.3/reference/cli/loop) |
| **Hooks and trust** | `hooks`, `trust` | [Hooks and trust](/docs/v0.3/reference/cli/hooks-trust) |
| **Configure appearance and behavior** | `config`, `list-themes`, `list-pets` | [Config](/docs/v0.3/reference/cli/config) |
| **Maintain and recover** | `coverage`, `workspace`, `reload`, `reset`, `gc`, `uninstall`, `ping` | [Maintenance](/docs/v0.3/reference/cli/maintenance) |

The [configuration guide](/docs/v0.3/customization/configuration) owns the file model behind `rimz config`; the [config reference](/docs/v0.3/reference/cli/config) covers the command mechanics.
...
**Addressing agents.** `message`, `transcript`, `pane`, and the `agents` management verbs all name agents the same way: `@<handle>` for who, `#<channel>` for which named lane, worktree, or in-place team, or a raw pane id. The one canonical explanation is [Addressing agents](/docs/v0.3/reference/cli/agents#addressing-agents).
...
**Exit codes carry the verdict.** A supervised [`agents … -p`](/docs/v0.3/reference/cli/agents#supervised-runs--p) run exits `0` completed, `1` failed, `124` timed out, `130` canceled, so a script branches without parsing output. Other commands follow the usual `0` success / non-zero error convention. [`rimz ping`](/docs/v0.3/reference/cli/maintenance#ping) prints `ok` for a liveness probe.
