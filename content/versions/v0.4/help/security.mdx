...
Room layout stays out of a repo's reach: a project config carrying a `[layout]` table (including tmux status `#(...)` and popup commands) is refused, with the fix to move it to your per-machine config. Any field RimZ can execute must enter this hash, and a unit test fails if one slips out. The mechanics, the grant record, and the diff are in [the trust internals](https://github.com/rimio-ai/rimz/blob/v0.4/docs/internals/harness/trust.md?ref=docs); the command is in [the trust reference](/docs/v0.4/reference/cli/hooks-trust#project-trust).
...
- **Reconfigure**: it applies the room's mouse options and, if you set one, binds the [focus key](/docs/v0.4/customization/configuration#sidebar-rendering), both at runtime without writing your `config.kdl`.
...
Hook payloads can carry prompts, tool inputs, file paths, command arguments, and errors. They stay in RimZ's local state, and nothing forwards them off the box. Payload-fidelity and retention controls (a `[privacy]` block with `payload_mode` and `retention_days`) are a planned project surface, tracked in [configuration.md](/docs/v0.4/customization/configuration) against the [hook adapter boundary](https://github.com/rimio-ai/rimz/blob/v0.4/docs/internals/agents/adapter.md?ref=docs#the-hook-path).
