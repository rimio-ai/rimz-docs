...
Key detail and the daemon-view placement live in [configuration → remote control](/docs/v0.4/customization/configuration#remote-control); which providers carry the surface is the `remote` row of the [coverage matrix](/docs/v0.4/reference/agent-support#the-coverage-matrix).

## See also

- [Web](/docs/v0.4/agents/web) — open the same room in a browser, locally or tunnelled from a server.
- [Agents](/docs/v0.4/agents/fleet) — what the room holds that you are reattaching to.
- [The sidebar](/docs/v0.4/agents/sidebar) — reading the link-health badge and the recovered column.
- [Troubleshooting](/docs/v0.4/help/troubleshooting) — a link that will not connect, a room that will not start, resetting state.
- [CLI reference](/docs/v0.4/reference/cli/remote) · [Configuration](/docs/v0.4/customization/configuration) — the `remote` command surface and `remote.toml`.
