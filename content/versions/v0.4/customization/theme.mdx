...
Zellij web rooms use the active scheme for the browser terminal when `[web.zellij] style_client` is true; see [Web CLI](/docs/v0.4/reference/cli/web).
...
`color` accepts a palette role, `#rrggbb`, or a raw index. Which blocks appear and in what order is a [Display](#display) and discovery setting (see [configuration.md → Provider dashboard](/docs/v0.4/customization/configuration#provider-dashboard)).
...
The [pets guide](/docs/v0.4/customization/pets) covers the rest: what the pet acts out, the built-in and [petdex.dev](https://petdex.dev/) catalogs, bring-your-own sprite sheets, the pixel and cell-art render tiers, and the offline and privacy story.
