...
RimZ marks only worktrees it creates, so it manages agent workspaces without claiming arbitrary checkouts. The marker, `.worktreeinclude` seeding, `.worktreelink` symlinks, and the `rimz gc` sweep are in [worktrees.md](https://github.com/rimio-ai/rimz/blob/v0.4/docs/internals/harness/worktrees.md?ref=docs).
