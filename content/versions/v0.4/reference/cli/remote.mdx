...
`--web-port <port>` pins the local browser origin; otherwise RimZ derives a stable port from the session name in `8300..8399`. The room itself is [`rimz web`](/docs/v0.4/reference/cli/web).

Link health, web tunneling, and reconnect mechanics are in [remote internals](https://github.com/rimio-ai/rimz/blob/v0.4/docs/internals/remote.md?ref=docs).
