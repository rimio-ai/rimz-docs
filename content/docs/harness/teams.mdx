...
`forge` is the team RimZ builds itself with, shipped ready to copy under [`examples/teams/forge/`](https://github.com/rimio-ai/rimz/blob/main/examples/README.md?ref=docs): three roles that carry one change through plan, code, and review, each role a profile with its own model, effort, and system-prompt file.
