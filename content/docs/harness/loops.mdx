---
title: "Loops"
description: "rimz loop is cron for your agents. A task is a name, a schedule, and one action: an agent turn in a real pane, a wake to an agent that is already running, or a shell…"
---
> `rimz loop` is cron for your agents. A task is a name, a schedule, and one action: an agent turn in a real pane, a wake to an agent that is already running, or a shell command that guards either. It fires while the room is open. The clock, the state files, and the run log underneath are in [loops.md](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/internals/harness/loops.md?ref=docs).
...
The ramp and retry keys are in [configuration.md → Resume](/docs/customization/configuration#resume); the full arm, fire, and exhaust state machine is [providers.md → Auto-continue](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/internals/agents/providers.md?ref=docs#auto-continue).
...
- [loops.md](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/internals/harness/loops.md?ref=docs): the clock, state files, and run log underneath.
