---
title: "Worktrees"
description: "Agents working in parallel need isolation, and Git already ships the primitive for it: a worktree (git worktree) is a second checkout of your repository, in its own…"
---
...
Neither file runs a command; sources are confined to the project root, and a pattern that matches nothing skips with a warning while the launch continues. The exact copy, symlink, and safety rules are in [the worktree internals](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/internals/harness/worktrees.md?ref=docs#seeding-worktreeinclude-and-worktreelink).
...
`dir` is a directory template: a relative path resolves from the repository root, and `{repo}` expands to its basename. `base` is `head` (branch from local `HEAD`, the default), `fresh` (branch from `origin/HEAD`), or any Git ref; `rimz worktree new --base <ref> --branch <name>` overrides both for a single tree. The full field reference is [configuration → worktrees](/docs/customization/configuration#worktrees), and the complete mechanics live in [the worktree internals](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/internals/harness/worktrees.md?ref=docs).
...
- [Worktree internals](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/internals/harness/worktrees.md?ref=docs) — the ownership marker, file seeding, and the landed-content proof.
