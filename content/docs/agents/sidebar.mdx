...
2. **Parked (`⏸`).** Blocked on the provider, not on you. There is nothing to answer until the limit recovers, and with [auto-continue](/docs/harness/loops#built-in-recovery) on, nothing to do at all; parks rank below the queue and above every calm card.
