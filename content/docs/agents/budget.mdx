...
One term collision to clear first: the `5h` and `7d` bars on the provider dashboard are also called budgets. Those are your subscription plan's included usage, metered by the provider and read-only to you ([budget is not spend](/docs/agents/insight#budget-is-not-spend)), though a [scheduled ping](/docs/harness/loops#prime-the-budget-window) can start those windows on your clock. Most of this page is about the dollar caps you set; the [last section](#spend-the-provider-surplus) reads the provider's budget instead, releasing scheduled work only when the week has surplus to spend.
...
- **The day does.** A `/day` park reopens at the next local midnight, and with [auto-continue](/docs/harness/loops#built-in-recovery) on, the continue prompt goes to the agents RimZ interrupted — never to agents that were already at rest.
...
The gate is a read, nothing more. The headroom comes from the provider's own usage reporting, the same account-scoped reading that draws the dashboard bars, cached on disk by the sessions you already run. Checking it runs no command and spends no tokens. A closed gate records `surplus skipped` in the task's run history with the reading it saw (`claude 7d window surplus 1.4x below 1.5x`), adds no [strike](/docs/harness/loops#built-in-recovery), and the schedule keeps polling until real surplus appears. It also fails closed: a missing, incomplete, expired, or not-yet-started window reading keeps the gate shut, so an API-key account, which has no subscription window to read, never fires a surplus-gated task.

The gate rides loop tasks only, on `--agent` and `--wake` actions, and it is evaluated before any `--check` guard, so a closed gate does not even run the check. The soak-task recipe is [loops → soak up the weekly surplus](/docs/harness/loops#soak-up-the-weekly-surplus); the exact flag grammar is the [loop reference](/docs/reference/cli/loop).
...
- [Loops](/docs/harness/loops#built-in-recovery) — auto-continue, task budgets, and hands-off recovery in one loop.
