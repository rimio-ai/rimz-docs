...
[examples/zellij/config.kdl](https://github.com/rimio-ai/rimz/blob/main/examples/zellij/config.kdl) is the whole baseline, the [essentials](/docs/getting-started/setup#zellij) plus every block below and the `tokyo-night` theme, as one file, since Zellij reads a single config. Starting fresh, copy it; with an existing `config.kdl`, lift the blocks you want. Unlisted keys keep Zellij's defaults either way.
