...
The file is `~/.config/zellij/config.kdl`. `zellij setup --dump-config` prints the full default set, and `zellij setup --check` validates your edits. Every key here is catalogued in the [Zellij upstream reference](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/externals/mux-adapter/zellij-reference.md?ref=docs#configuration).

[examples/zellij/config.kdl](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/zellij/config.kdl) is the whole baseline, the [essentials](/docs/getting-started/setup#zellij) plus every block below and the `tokyo-night` theme, as one file, since Zellij reads a single config. Starting fresh, copy it; with an existing `config.kdl`, lift the blocks you want. Unlisted keys keep Zellij's defaults either way.
...
The file is `~/.tmux.conf` (or `~/.config/tmux/tmux.conf`); reload it with `tmux source-file ~/.tmux.conf` or the `prefix` + `r` binding below. Every option here is catalogued in the [tmux upstream reference](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/externals/mux-adapter/tmux-reference.md?ref=docs#options).

tmux is the spartan one out of the box: no mouse, a short scrollback, and thin, untitled pane borders. The blocks below ship as four self-contained modules under [examples/tmux/](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/README.md?ref=docs#tmux--tmux), [`agents.conf`](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/tmux/agents.conf) (the [essentials](/docs/getting-started/setup#tmux)), [`quality-of-life.conf`](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/tmux/quality-of-life.conf) (copy-mode, window names, splits), [`zellij-keys.conf`](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/tmux/zellij-keys.conf) (the parity chords), and [`theme-tokyonight.conf`](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/tmux/theme-tokyonight.conf) (frames and status bar), so your `~/.tmux.conf` stays yours and adopts by reference:
...
- [examples/](https://github.com/rimio-ai/rimz/blob/v0.4.2/examples/README.md?ref=docs): the shipped baseline files these blocks come from.
...
- [Zellij](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/externals/mux-adapter/zellij-reference.md?ref=docs) and [tmux](https://github.com/rimio-ai/rimz/blob/v0.4.2/docs/externals/mux-adapter/tmux-reference.md?ref=docs) upstream references: every option catalogued at the source.
